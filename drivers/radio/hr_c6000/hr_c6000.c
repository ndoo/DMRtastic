...
static int hr_c6000_ctrl_write(const struct device *dev, uint8_t page, uint8_t reg, uint8_t val)
{
...
static int hr_c6000_ctrl_read(const struct device *dev, uint8_t page, uint8_t reg, uint8_t *val)
{
	const struct hr_c6000_config *cfg = dev->config;
...
static int hr_c6000_ctrl_write_pairs(const struct device *dev, uint8_t page,
				     const struct hr_c6000_reg_pair *pairs, size_t count)
{
	int rc;
...
static int hr_c6000_ctrl_write_array(const struct device *dev, uint8_t page, uint8_t reg,
...
static int hr_c6000_run_init_sequence(const struct device *dev)
{
	...
	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_init_pll_regs,
				       ARRAY_SIZE(hr_c6000_init_pll_regs));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_init_setup_regs,
				       ARRAY_SIZE(hr_c6000_init_setup_regs));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_array(dev, HRC6000_PAGE_CTRL, 0x11, hr_c6000_init_ctrl_0x11_array,
				       sizeof(hr_c6000_init_ctrl_0x11_array));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_init_vocoder_regs,
	...
	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_init_post_flush_regs,
				       ARRAY_SIZE(hr_c6000_init_post_flush_regs));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_array(dev, HRC6000_PAGE_AUX, 0x10, hr_c6000_init_aux_0x10_array,
				       sizeof(hr_c6000_init_aux_0x10_array));
	if (rc < 0) {
		return rc;
	}
	rc = hr_c6000_ctrl_write_array(dev, HRC6000_PAGE_AUX, 0x30, hr_c6000_init_aux_0x30_array,
				       sizeof(hr_c6000_init_aux_0x30_array));
	if (rc < 0) {
		return rc;
	}
	rc = hr_c6000_ctrl_write_array(dev, HRC6000_PAGE_AUX, 0x40, hr_c6000_init_aux_0x40_array,
				       sizeof(hr_c6000_init_aux_0x40_array));
	if (rc < 0) {
		return rc;
	}
	rc = hr_c6000_ctrl_write_array(dev, HRC6000_PAGE_AUX, 0x50, hr_c6000_init_aux_0x50_array,
				       sizeof(hr_c6000_init_aux_0x50_array));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_AUX, hr_c6000_init_aux_more_regs,
				       ARRAY_SIZE(hr_c6000_init_aux_more_regs));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_init_late_regs,
				       ARRAY_SIZE(hr_c6000_init_late_regs));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_AUX, hr_c6000_init_aux_cleanup_regs,
				       ARRAY_SIZE(hr_c6000_init_aux_cleanup_regs));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_init_rxprep_regs,
				       ARRAY_SIZE(hr_c6000_init_rxprep_regs));
	if (rc < 0) {
		return rc;
	}

	rc = hr_c6000_ctrl_write_array(dev, HRC6000_PAGE_AUX, 0x04, hr_c6000_ms_sync_pattern,
	...
	return hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_init_final_regs,
...
static int hr_c6000_api_set_fm_rx(const struct device *dev)
{
	...
	rc = hr_c6000_ctrl_write_pairs(dev, HRC6000_PAGE_CTRL, hr_c6000_set_fm_rx_regs,
...
static int hr_c6000_api_set_audio_path(const struct device *dev, bool fm_feedthrough)
{
	struct hr_c6000_data *data = dev->data;
	int rc;
	...
	k_mutex_lock(&data->bus_lock, K_FOREVER);
	rc = hr_c6000_ctrl_write(dev, HRC6000_PAGE_CTRL, HRC6000_REG_36_FM_FEED,
				 fm_feedthrough ? HRC6000_36_FM_FEED_ON : HRC6000_36_FM_FEED_OFF);
	k_mutex_unlock(&data->bus_lock);
	return rc;
}
...
static int hr_c6000_api_set_fm_tx(const struct device *dev, const struct radio_bb_fm_tx_cfg *cfg)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(cfg);
	return -ENOTSUP;
...
static int hr_c6000_api_set_dmr(const struct device *dev, const struct radio_bb_dmr_cfg *cfg)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(cfg);
	return -ENOTSUP;
...
{
	ARG_UNUSED(dev);
	ARG_UNUSED(enable_mic);
	return -ENOTSUP;
}

static int hr_c6000_api_get_rx_ids(const struct device *dev, uint32_t *tg_id, uint32_t *src_id)
{
	ARG_UNUSED(dev);
	if (tg_id) {
		*tg_id = 0;
...
static int hr_c6000_api_get_rx_color_code(const struct device *dev, uint8_t *cc)
{
	ARG_UNUSED(dev);
	if (cc) {
		*cc = 0;
...
static int hr_c6000_api_set_dmr_ids(const struct device *dev, uint32_t tg_id, uint32_t src_id)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(tg_id);
	ARG_UNUSED(src_id);
...
static int hr_c6000_api_set_talker_alias(const struct device *dev, const char *text)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(text);
	return -ENOTSUP;
...
static int hr_c6000_api_set_talker_alias_location(const struct device *dev, int32_t lat_1e7,
						  int32_t lon_1e7)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(lat_1e7);
...
static int hr_c6000_api_inject_ambe_frame(const struct device *dev, const uint8_t *frame)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(frame);
	return -ENOTSUP;
...
static int hr_c6000_api_get_sync_state(const struct device *dev, bool *has_sync, bool *cc_held,
...
static int hr_c6000_api_write_raw(const struct device *dev, uint8_t page, uint8_t reg, uint8_t val)
{
	return hr_c6000_ctrl_write(dev, page, reg, val);
}

static int hr_c6000_api_read_raw(const struct device *dev, uint8_t page, uint8_t reg, uint8_t *val)
{
...
static int hr_c6000_init(const struct device *dev)
{
	...
	LOG_INF("HR-C6000 init OK; IF readback 0x%02X 0x%02X 0x%02X", r07, r08, r09);
	return 0;
}
...
#define HRC6000_INIT(inst)                                                                         \
	static const struct hr_c6000_config hr_c6000_cfg_##inst = {                                \
		...
		.rf_recv_iq_mode = !strcmp(DT_INST_PROP_OR(inst, rf_recv_mode, "if"), "iq"),       \
	};                                                                                         \
	static struct hr_c6000_data hr_c6000_data_##inst;                                          \
	DEVICE_DT_INST_DEFINE(inst, hr_c6000_init, NULL, &hr_c6000_data_##inst,                    \
			      &hr_c6000_cfg_##inst, POST_KERNEL,                                   \
			      CONFIG_RADIO_HR_C6000_INIT_PRIORITY, &hr_c6000_api);
