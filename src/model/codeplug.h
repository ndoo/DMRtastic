...
/* Format (modern 80ch/176B vs legacy 16ch/48B) is runtime-detected, not address-based; channels[] holds the modern superset. */
...
/* value: CTCSS -- tenths of Hz (2035 = 203.5 Hz). DCS -- octal-digit-packed code (print with "%03o").
 * inverted: DCS-only, true for the inverted variant. */
struct cp_css {
...
/* Durations sub-view (cp_signalling_dtmf_durations) is a separate on-flash region, not a slice of this struct. */
...
/* Best-effort decode, valid only when magic == CP_NV_SETTINGS_MAGIC_LATEST -- shape isn't modeled for older magics. */
...
/* slot_index_0based is a physical slot (0..67), not "the nth populated zone" -- see codeplug_get_zone_inuse_bitmap(). */
