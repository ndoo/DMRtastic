...
struct radio_trx_api {
	...
	/** Last RX frequency set via set_frequency(dev, hz, false); *freq_hz is 0 if never
	 * RX-tuned.
	 */
	...
	int (*check_css)(const struct device *dev, uint16_t tone, enum radio_css_type type,
			 bool *detected);

	int (*get_rssi)(const struct device *dev, uint8_t *signal, uint8_t *noise);
	...
	int (*set_voice_channel)(const struct device *dev, enum radio_voice_ch ch, uint8_t gain,
				 uint16_t deviation);

	int (*set_tone1_freq)(const struct device *dev, uint16_t tone_freq_val);

	int (*register_squelch_cb)(const struct device *dev, radio_trx_squelch_fn_t fn);
	int (*register_css_cb)(const struct device *dev, radio_trx_css_fn_t fn);
	...
	int (*write_raw)(const struct device *dev, uint8_t reg, uint8_t hi, uint8_t lo);
	int (*read_raw)(const struct device *dev, uint8_t reg, uint8_t *hi, uint8_t *lo);
};
